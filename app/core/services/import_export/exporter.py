...
def export_users_to_json(output_path: str) -> None:
    ...
    users_data = []
    for user in users:
        users_data.append({col: getattr(user, col) for col in user.__table__.columns})
    try:
        with open(output_path, "w", encoding="utf-8") as json_file:
            dump(users_data, json_file, indent=4)
        logger.info(f"Successfully exported {len(users)} users to {output_path}.")
    except Exception as e:
        logger.exception(f"Failed to export users to JSON: {e}")
...
def export_users_to_csv(output_path: str) -> None:
    ...
    try:
        ...
        logger.exception(f"Failed to export users to CSV: {e}")


def export_links_to_json(output_path: str, author_id: int | None = None) -> None:
    links = link_service.get_links_by_author(author_id) if author_id is not None else link_service.get_links()
    links_data = []
    for link in links:
        link_data = {col: getattr(link, col) for col in link.__table__.columns}
        if link.author:
            link_data["author"] = {col: getattr(link.author, col) for col in link.author.__table__.columns}
    ...
    try:
        with open(output_path, "w", newline="", encoding="utf-8") as csv_file:
            ...
            for link in track(links, description="Exporting links..."):
                row = {col: getattr(link, col) for col in link.__table__.columns}
                if link.author:
